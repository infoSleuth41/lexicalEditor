...
import { DecoratorNode, createCommand, type LexicalCommand, $getNodeByKey } from "lexical";
...
export const INSERT_POLL_COMMAND: LexicalCommand<{ question: string }> = createCommand(
    "INSERT_POLL_COMMAND"
);
...
function PollComponent({
    ...
    const updateNode = (updater: (node: PollNode) => void) => {
        editor.update(() => {
    ...
    const handleVote = (id: string) => {
        updateNode((node) => {
            const opts = node.__options.map((o) => {
                if (o.id === id) return { ...o, votes: o.votes + (votedId === id ? -1 : 1) };
    ...
    const handleAddOption = () => {
        ...
        if (!text) return;
        updateNode((node) => {
            node.__options = [...node.__options, { id: makeOptionId(), text, votes: 0 }];
        });
        setNewOptionText("");
    };
    ...
    const handleRemoveOption = (id: string) => {
        updateNode((node) => {
            node.__options = node.__options.filter((o) => o.id !== id);
        });
    };

    return (
        <div className="my-2 max-w-md rounded-lg border bg-card p-4 shadow-sm">
                        ...
                        <div key={option.id} className="group flex items-center gap-2">
                            <button
                                ...
                                className={`relative flex-1 overflow-hidden rounded-md border px-3 py-2 text-left text-sm transition-colors ${
                                    isVoted ? "border-primary" : "border-input"
                                }`}
                            >
                                <div
            ...
            <p className="mt-2 text-xs text-muted-foreground">{totalVotes} vote{totalVotes === 1 ? "" : "s"}</p>
        </div>
    );
}
...
export class PollNode extends DecoratorNode<React.ReactNode> {
    ...
    constructor(question: string, options?: PollOption[], key?: NodeKey) {
        ...
        this.__options = options ?? [
            { id: makeOptionId(), text: "Option 1", votes: 0 },
            { id: makeOptionId(), text: "Option 2", votes: 0 },
        ];
    }
